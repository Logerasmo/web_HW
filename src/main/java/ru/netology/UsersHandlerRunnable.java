        ...
        try (final var serverSocket = new ServerSocket(9999)) {
            while (true) {
                try (final var socket = serverSocket.accept();
                    ...
                    } else request = new Request(requestLine, headers);
                    String key = method + path.substring(0, path.indexOf("?"));
                    if (server.handlers.get(key) == null) {
                        badRequest(out);
                        continue;
                    }
                    server.handlers.get(key).handle(request, out);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
